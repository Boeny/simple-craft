...
module.exports = {
	...
	initialProcess: function(){
		...
		this.plains = [
			{x:0, y:-1, l:this.size.y, restore: 1},
	...
	pointsCollision: function(i, p){
		...
		for (var j=i+1; j<this.points.length; j++){
			...
			if (l < this.collision_distance && vdot(p.speed, n) < 0)
			{
				l = -(p2.restore + p.restore)/2;
				
				vadd(p, vset(n, this.collision_distance-l, true));
				vadd(p.speed, vmult(n, vdot(p.speed, n) * l, true));
				
				vmult(n, -1);
				vadd(p2.speed, vmult(n, vdot(p2.speed, n) * l, true));
			}
		}
	},
